...
var userStorage = {
  ...
  setLatestAuthType(authType) {
    if (authType === "twitterv1") {
      encryptedStorage_default.setItem("latest_auth_type", "twitter");
    } else {
...
function isTron(chain) {
  ...
  if (typeof chain === "string" || typeof chain === "number") {
    ...
    chainInfo = chains.getEVMChainInfoById(chainId);
  } else {
    chainInfo = chains.getEVMChainInfoById(chain.id);
  }
...
var getSocialAuthUrl = async (parms) => {
  ...
  if (authType === "twitter") {
    ...
    return `${config_default.env.authCore}/oauth.html?data=${data}`;
  }
...
var connect = async (param) => {
  ...
  let chainInfo;
  if (param.chain) {
    chainInfo = chains2.getChainInfo(param.chain);
  ...
  if ("phone" in param && "code" in param && param.phone && param.code) {
    ...
    userInfo = await pnApi_default.loginWithJWT(param.jwt);
    latestAuthType = "jwt" /* jwt */;
  } else if ("socialType" in param && param.socialType && "code" in param && param.code && "nonce" in param && param.nonce) {
...
var EthereumProvider = class {
  constructor() {
    ...
    __privateAdd(this, _events, new EventEmitter2());
    ...
    if (localChainId && chains3.getEVMChainInfoById(localChainId)) {
    ...
    eventUtils_default.on("particleAuthConnect" /* ParticleAuthConnect */, () => {
      if (this.selectedAddress) {
        __privateGet(this, _events).emit("connect", { chainId: intToHex(__privateGet(this, _chainId)) });
        __privateGet(this, _events).emit("accountsChanged", [this.selectedAddress]);
      }
    });
    eventUtils_default.on("connectWithChain", (chainInfo) => {
      if (chainInfo.chainType === "evm") {
        ...
        setCurrentEvmChainId(__privateGet(this, _chainId));
        __privateGet(this, _events).emit("chainChanged", intToHex(__privateGet(this, _chainId)));
      }
    });
  }
  get selectedAddress() {
  ...
  get chainId() {
    return intToHex(__privateGet(this, _chainId));
  }
  isConnected() {
    ...
    if (this.selectedAddress) {
      ...
      if (userStorage_default.isLogin()) {
        ...
        await new Promise((resolve, reject) => {
          eventUtils_default.emit("requestEthereumAccount" /* RequestEthereumAccount */, { resolve, reject, param });
        });
        if (!userStorage_default.isLogin()) {
  ...
  async sendTransaction(transaction) {
    ...
    if (isNullish(transaction.type)) {
      const chain2 = chains3.getEVMChainInfoById(chainId);
      if (chain2 && chains3.isChainSupportEIP1559(chain2)) {
  ...
  async signMessage(message, uniq) {
    ...
    return concatSig(toBuffer2(sig.v + 27), sig.r, sig.s);
  }
  async signTypedData({
    data,
    version,
  ...
  async switchChain(chainId) {
    if (Number(chainId) !== __privateGet(this, _chainId)) {
      if (!chains3.getEVMChainInfoById(Number(chainId))) {
      ...
      setCurrentEvmChainId(__privateGet(this, _chainId));
      setTimeout(() => __privateGet(this, _events).emit("chainChanged", intToHex(__privateGet(this, _chainId))), 0);
    }
    return null;
  }
};
...
import { providerErrors as providerErrors2 } from "@metamask/rpc-errors";
...
var SolanaWallet = class {
  constructor() {
    ...
    __privateAdd(this, _events2, new EventEmitter3());
    this.isParticleNetwork = true;
    this.name = "Particle";
    ...
    if (localChainId && chains4.getSolanaChainInfoById(localChainId)) {
    ...
    eventUtils_default.on("particleAuthConnect" /* ParticleAuthConnect */, async () => {
      if (this.selectedAddress) {
        ...
        __privateGet(this, _events2).emit("accountsChanged", [this.selectedAddress]);
      }
    });
    eventUtils_default.on("connectWithChain", (chainInfo) => {
  ...
  async connect(param) {
    try {
      ...
      if (userStorage_default.isLogin()) {
        ...
        await new Promise((resolve, reject) => {
          eventUtils_default.emit("requestSolanaAccount" /* RequestSolanaAccount */, { resolve, reject, param });
        });
        if (!userStorage_default.isLogin()) {
  ...
  async switchChain(chainId) {
    ...
    if (!chains4.getSolanaChainInfoById(chainId)) {
      throw providerErrors2.chainDisconnected("The chain is not supported.");
    }
    const userInfo = userStorage_default.getUserInfo();
    if (userInfo) {
      if (!this.selectedAddress) {
...
import sha2563 from "crypto-js/sha256.js";
...
function hashPassword(pwd) {
  ...
  return sha2563(userInfo.uuid + pwd).toString().toLowerCase();
}
...
export {
  ...
  syncUserInfo,
  thirdpartyAuth,
  fromHex as tronAddressFromHex,
  toHex as tronAddressToHex,
